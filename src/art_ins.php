...
class art_in {
    ...
    { // id - > Какой сейчас ход || $xy - > Координаты первого хода (Обрабатывается если $id = 2)
        ...
        // Удалим . , .. и 0.db
            ...
            foreach ($directs as $w) {
                if ($w[0] == '.' || $w[0] == '0'){ 
                    unset($directs[$t]); $t--; 
                    $this->mass = array_values($directs);
                }
    ...
    private function helper($id , $xy){                              // Проверяет подходит ли нам файл, и в этом случае возвращает нам предполагаемый ход
        ...
        if ($past_step == $xy) {                                            // Если координаты в файле и хода опонента совпадают
            ...
            if ($this->pole[$new_step[0]][$new_step[1]] == '-') {
    ...
    private function analizator($id, $xy)                                // Обработка данных в БД и вынесение вердикта какую службу вызывать
    {                                                                       // $xy - ход аппонента
        $this->q = 1;                                                       // Счётчик
        $bufer = null;
    ...
    private function rand_step($pole , $N)                                   // Случайный выбор координат
    {                               
    ...
    public function main($pole, $xy, $id)                                        // Управляющая ИИ конструкция
    {
        $this->pole = $pole;

        if ($id < 3) {
            $this->mass = $this->worker($id, $xy);                                 // Получить массив из базы данных
        }
    
        $bufer = $this->analizator($id , $xy);                              // Вызвать анализатор
    
        if ($bufer == null) {
            return $this->rand_step($pole, 3);                             // Если анализатор вернёт NULL, Запуск рандома
        }                                                                   
        else { 
            return $bufer;                                                 
        }
